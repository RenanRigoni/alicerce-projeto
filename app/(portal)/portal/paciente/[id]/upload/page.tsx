...
export default function UploadFamiliaPage() {
  ...
  async function handleEnviar() {
    ...
    if (!res.ok) {
      setErro(json.error ?? 'Erro ao enviar o arquivo. Tente novamente.')
      return
    }
  ...
  return (
    <div className="space-y-6 max-w-md">
      <div className="flex items-center gap-3">
        <a href={`/portal/paciente/${pacienteId}?aba=documentos`} className="text-sm transition-colors hover:opacity-70" style={{ color: 'var(--color-ink-soft)' }}>
          ← Voltar
        </a>
