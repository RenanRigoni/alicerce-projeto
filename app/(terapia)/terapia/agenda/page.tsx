...
export default async function AgendaPage() {
  ...
  const [
    ...
    // Meus pacientes ativos com horários
    supabase
      .from('paciente_terapeutas')
      .select('pacientes(id, nome, status, horarios_atendimento)')
      .eq('terapeuta_id', user!.id),
    // Eventos excepcionais (devolutivas, reuniões, etc) — não inclui sessão recorrente
    supabase
      .from('agendamentos')
  ...
  // Gera sessões recorrentes: 3 meses atrás até 9 meses à frente
