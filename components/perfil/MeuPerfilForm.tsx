...
export function MeuPerfilForm(props: Props) {
  const { userId, email, cpf, role, criadoEm, conselhoTipo, tipoProfissional } = props
  ...
  function cancelar() {
    setNome(props.nome); setTelefone(props.telefone ?? ''); setDataNasc(props.dataNascimento ?? '')
    setRg(props.rg ?? ''); setSexo(props.sexo ?? ''); setEstadoCivil(props.estadoCivil ?? '')
    setConselhoNum(props.conselhoNumero ?? ''); setConselhoUf(props.conselhoUf ?? '')
    setCbo(props.cboCodigo ?? ''); setEspecialidade(props.especialidade ?? ''); setBiografia(props.biografia ?? '')
    setEditando(false)
  }
  ...
  return (
    <div className="space-y-5">
      ...
      >
        {/* Header: avatar + nome + cargo + botão editar */}
        <div
        ...
        >
          ...
          <div className="relative shrink-0">
            ...
            {editando && (
              ...
              >
                {uploadando ? <Loader2 size={20} className="animate-spin text-white" /> : <Camera size={20} className="text-white" />}
              </button>
            )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-lg font-semibold truncate" style={{ fontFamily: 'var(--font-lora)', color: 'var(--color-ink)' }}>
                  {nome}
                </div>
                <div
        ...
        <div className="px-6 py-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
            ...
            {/* E-mail */}
            <CampoLocked label="E-mail" value={email} />
      ...
      {/* ── Card informações profissionais (terapeuta) ── */}
      {role === 'terapeuta' && (
        <div
          className="rounded-2xl overflow-hidden"
          style={{ border: '1px solid var(--color-border-soft)', background: 'var(--color-warm-white)' }}
          ...
          <div className="px-6 py-5 space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <CampoLocked
                ...
                value={tipoConfig?.label ?? tipoProfissional ?? null}
              />

              {/* Conselho */}
              <CampoLocked
                label="Conselho"
                value={conselhoTipo ?? tipoConfig?.conselho ?? null}
              />
              ...
              <Campo label={`Nº ${tipoConfig?.conselho ?? 'Conselho'}`} editando={editando}>
                {editando ? (
                  <input
                    value={conselhoNum}
                    onChange={e => setConselhoNum(e.target.value)}
            ...
            <Campo label="Biografia" editando={editando}>
              {editando ? (
                ...
                <div
                  ...
                  style={{ color: biografia ? 'var(--color-ink)' : 'var(--color-ink-faint)' }}
                >
...
function Campo({ label, editando, children }: { label: string; editando: boolean; children: React.ReactNode }) {
  return (
    <div>
      <div
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between gap-2">
        <div
          className="text-sm"
          style={{ color: value ? 'var(--color-ink)' : 'var(--color-ink-faint)' }}
        >
          {value || 'Não informado'}
        </div>
...
function Val({ children, empty = '—' }: { children?: string | null; empty?: string }) {
  return (
    <div
      className="text-sm"
      style={{ color: children ? 'var(--color-ink)' : 'var(--color-ink-faint)' }}
    >
      {children || empty}
    </div>
  )
